...
test('given a string as input, returns a new array where every element is a character of the input string', () => {
...
test('takes in the recipe and returns a new array of the food items without any amount or units. Just the name', () => {
  expect(listFoods(gruffaloCrumble)).toStrictEqual(['Gruffalo', 'oats', 'brown sugar', 'flour', 'pure maple syrup', 'chopped nuts', 'baking soda', 'baking powder', 'cinnamon', 'melted butter', 'fresh water']);
});

test('takes in the recipe and extracts the action verbs from the steps', () => {
  expect(stepActions(gruffaloCrumble)).toStrictEqual(['Pre-heat', 'De-prickle', 'Sprinkle', 'Mix', 'Grease', 'Combine', 'Fold', 'Spread', 'Bake']);
});

test('removes characters from string and other stuff', () => {
...
test('given a string of comma-separated values (CSV) as input. returns the total sum of the numeric values', () => {
  expect(totalSumCSV('1, 4, 5, 7, 2')).toStrictEqual(19);
  expect(totalSumCSV('147')).toStrictEqual(147);
});

test('takes in a string and returns a new string where all the vowels of the original string have been removed', () => {
